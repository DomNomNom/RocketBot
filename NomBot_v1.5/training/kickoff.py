...
class GradeBasedOnKickoff(Grader):
    ...
    def on_tick(self, tick: TrainingTickPacket) -> Optional[Grade]:
        ...
        if packet.game_info.seconds_elapsed - self.first_time > 6 and abs(ball_y) > 4000:
...
class NaturalKickoffExercise(TrainingExercise):
    ...
    def make_game_state(self, rng: SeededRandomNumberGenerator) -> GameState:
        return GameState(
            ...
            cars={
                0: CarState(boost_amount=95, jumped=True, double_jumped=True, physics=Physics(
                    location=Vector3(200, -5400, 100),
                    rotation=Rotator(0, 3, 0),
                    velocity=Vector3(0, 0, 0),
                    angular_velocity=Vector3(0, 0, 0),
                )),
                1: CarState(boost_amount=95, jumped=True, double_jumped=True, physics=Physics(
                    location=Vector3(-200, -5400, 100),
                    rotation=Rotator(0, 3, 0),
                    velocity=Vector3(0, 0, 0),
                    angular_velocity=Vector3(0, 0, 0),
